 ...
 */
class BatchDriverActor(tickInterval:FiniteDuration) extends Actor with ActorLogging with FeederExtensionActor {

  log.info(s"Starting feeder actor ${self.path}")

  import BatchDriverActor.SendNextLine
...
object BatchDriverActor {
  def props(tickInterval:FiniteDuration) = Props(new BatchDriverActor(tickInterval))
